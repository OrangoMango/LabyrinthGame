...
public class Menu {
	...
	public Menu(Stage stage) {
		...
		canvas.setOnMousePressed(e -> {
			...
			} else if (!this.openedWindow && isContainedIn(x, y, 550, 250, 600, 300)) {
				NewsScreen ns = new NewsScreen(this.stage);
			}
		});
		...
		canvas.setOnMouseMoved(e -> {
			...
			} else if (isContainedIn(x, y, 550, 250, 600, 300)) {
				if (!isIn) {
					updateCanvas(this.gc, 5);
					isIn = true;
				}
			} else if (isIn) {
				isIn = false;
	...
	private void updateCanvas(GraphicsContext gc, int ds) {
		...
		gc.drawImage(new Image("file://" + changeSlash(PATH) + ".labyrinthgame/Images/editor/button_news.png"), 550, 250, 50, 50);
		gc.setEffect(null);
	}
